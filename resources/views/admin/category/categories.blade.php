												...
												<tbody class="fw-semibold text-gray-600">
													@forelse ($categories as $category)
														<tr>
																	...
																	<div class="ms-5">
																		...
																		<div class="text-muted fs-7 fw-bold">
																			...
																			@if(isset($category->is_active))
																				• Durum: {{ $category->is_active ? 'Aktif' : 'Pasif' }}
																			@endif
																		</div>
																	</div>
																</div>
															...
															<td>
																<div class="badge badge-light-{{ $category->parent_id ? 'primary' : 'success' }}">
															...
															<td>
																@if($category->users && $category->users->count() > 0)
																	@foreach($category->users as $user)
																		<span class="badge badge-light-info me-1 mb-1">{{ $user->name_surname }}</span>
																	@endforeach
																@else
											...
											<div class="d-flex justify-content-between align-items-center mt-5">
												<div>
													<span class="text-muted">
														{{ $categories->firstItem() }} - {{ $categories->lastItem() }} arası gösteriliyor / Toplam: {{ $categories->total() }}
													</span>
												</div>
												<div>
							...
							<div class="flex-row-fluid py-lg-5 px-lg-15">
								...
								<form class="form" method="POST" action="{{ route('admin.categories.store') }}" enctype="multipart/form-data">
									...
									<div class="fv-row mb-7">
										...
										<input type="text" class="form-control form-control-solid" name="name" placeholder="Kategori adı" required />
									</div>
								
									<div class="fv-row mb-7">
										<label class="fs-6 fw-semibold mb-2">Açıklama</label>
										<textarea class="form-control form-control-solid" name="description" rows="3" placeholder="Açıklama..."></textarea>
									</div>
								
									<div class="fv-row mb-7">
										<label class="fs-6 fw-semibold mb-2">Sıralama</label>
										<input type="number" class="form-control form-control-solid" name="sort_order" placeholder="0" value="0" />
									</div>
									...
									<div class="fv-row mb-7">
										<label class="fs-6 fw-semibold mb-2">Kategori Görseli</label>
										<input type="file" class="form-control form-control-solid" name="image" accept=".png, .jpg, .jpeg" />
										<small class="text-muted">Sadece .png, .jpg, .jpeg uzantıları</small>
									</div>
								
									<div class="text-end">
										<button type="submit" class="btn btn-primary">
											<span class="indicator-label">Kategori Oluştur</span>
